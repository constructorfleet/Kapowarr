...
class UsenetDownload(ExternalDownload, BaseDirectDownload):
    "For downloading an NZB file via a Usenet client (e.g. SABnzbd)"
    ...
    def __init__(
        ...
        # Derive a job name from the NZB URL or from the renamed issue name
        nzb_basename = unquote_plus(splitext(basename(
            download_link.split('?')[0]
        ))[0]) or 'nzb_download'
        ...
        if not self._filename_body:
            self._filename_body = nzb_basename

        self._title = basename(self._filename_body)
        # SABnzbd will put the completed files in its own complete folder;
        # _files is updated with the actual storage path in update_status().
        self._files: List[str] = [self._title]
        return
    ...
    def update_status(self) -> None:
        ...
        # When SABnzbd reports a storage path the job has finished;
        # update _files so that post-processing can find the downloaded content.
        storage = nzb_status.get('storage', '')
        if storage:
            self._files = [storage]
